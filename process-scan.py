...
def get_images(directory):
    ...
    for f in listdir(directory):
        if isfile(join(directory, f)) and f.endswith("jpg"):
...
if stage_add_photos:
    chunk = doc.addChunk()
    chunk.addPhotos(get_images(image_dir))
...
if stage_align_cameras:
    ...
    options = dict(#accuracy=Metashape.HighAccuracy,
        ...
        tiepoint_limit=4000,
        filter_mask=True,
#        workitem_size_cameras=args.workitem,
#        max_workgroup_size=args.workitem*5
    )
...
print(doc.chunk.orthomosaic.projection.matrix)
print(doc.chunk.orthomosaic.meta)

doc.chunk.exportPoints(join(path, "sparse.obj"),
